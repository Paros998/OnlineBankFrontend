...
export const EditClientDataValidationSchema = Yup.object().shape({
    ...
    postalCode: Yup.string().required('Kod pocztowy jest wymagany').min(5, 'Podaj poprawny kod pocztowy'),
    homeAddress: Yup.string().required('Adres korespondencyjny jest wymagany'),
    city: Yup.string().required('Nazwa miasta jest wymagana'),
    secPostalCode: Yup.string().required('Kod pocztowy jest wymagany').min(5, 'Podaj poprawny kod pocztowy'),
